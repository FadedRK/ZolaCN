...
static NSUInteger ZLCNRecallTimeHookCount = 0;
...
static void ZLCNRecallTimeDoubleReplacement(id self, SEL _cmd, double value) {
    if (ZLCNShouldShieldRecallState()) {
        ...
        ZLCNShieldLog(@"BLOCK set_recallTime: double | Class=%@ | value=%f | blocked=%lu",
...
static void ZLCNRecallTimeLongLongReplacement(id self, SEL _cmd, long long value) {
    if (ZLCNShouldShieldRecallState()) {
        ZLCNRecallTimeBlockedCount++;
        ZLCNShieldLog(@"BLOCK set_recallTime: long long | Class=%@ | value=%lld | blocked=%lu",
                      NSStringFromClass(object_getClass(self)), value,
                      (unsigned long)ZLCNRecallTimeBlockedCount);
        return;
    }
...
static void ZLCNRecallTimeULongLongReplacement(id self, SEL _cmd, unsigned long long value) {
    if (ZLCNShouldShieldRecallState()) {
        ZLCNRecallTimeBlockedCount++;
        ZLCNShieldLog(@"BLOCK set_recallTime: unsigned long long | Class=%@ | value=%llu | blocked=%lu",
                      NSStringFromClass(object_getClass(self)), value,
                      (unsigned long)ZLCNRecallTimeBlockedCount);
        return;
    }
...
static void ZLCNInstallRecallTimeShield(void) {
    ...
    SEL sel = sel_registerName("set_recallTime:");
    SEL camelSel = sel_registerName("setRecallTime:");
    ...
    for (int i = 0; i < classCount && !ZLCNRecallTimeInstalled; i++) {
        Class cls = classes[i];
        Method method = ZLCNShieldDirectMethod(cls, sel);
        SEL matchedSEL = sel;
        if (!method) {
            method = ZLCNShieldDirectMethod(cls, camelSel);
            matchedSEL = camelSel;
        }
        if (!method) continue;
        ...
        if (strcmp(types, "v24@0:8d16") == 0) {
            ...
            ZLCNShieldLog(@"SKIP recall state setter | unsupported Types=%s", types);
            continue;
        }

        ZLCNRecallTimeClass = cls;
        ZLCNRecallTimeInstalled = YES;
        ZLCNRecallTimeHookCount = 1;
...
/* Called by the MediaStore layer immediately before it suppresses a remote recall.
   The short window is intentionally narrow: it only protects the in-memory recall
   state mutation that follows the same recall transaction. */
...
void ZLCNRecallStateShieldInstall(void) {
